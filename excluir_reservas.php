...
if (isset($_GET['id'])) {
	...
	$consulta = $sql->query("SELECT * FROM carros WHERE id = $id");
	...
	if ($consulta->num_rows == 1) {
		...
		if (isset($_POST['confirmarExclusao'])) {
			...
			$sql->query("UPDATE carros SET excluido = 1 WHERE id = $id");
...
<body>
	<h1>Excluir Carro</h1>
	<form method="post">
		<p>Você tem certeza que deseja excluir esse Carro <strong><?php echo $linha['modelo']; ?></strong>?</p>
		<input type="submit" name="confirmarExclusao" value="Sim">
		<button type="button" onclick="window.location.href='tabela_carros.php'">Não</button>
	</form>
</body>
</html>